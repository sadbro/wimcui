 ...
 * Every generator should use ref() instead of raw `names[id]` lookups.
 *
 *   const r = ref(names, warnings, "my_ec2");
 *   r("aws_vpc", cfg.vpcId, "id")       → "aws_vpc.my_vpc.id"  or null + warning
 *   r.name(someId)                        → "my_vpc"              or "MISSING" + warning
...
function createRefResolver(names, warnings, sourceLabel) {
  ...
  ref.list = (tfType, ids, prop) => {
    if (!ids || ids.length === 0) return [];
    return ids
      .map((id) => ref(tfType, id, prop))
      .filter(Boolean);
  };
...
const generators = {
  ...
  Subnet(nodes, names, ctx, warnings) {
    return nodes.map((n) => {
      ...
      const vpcRef = r("aws_vpc", cfg.vpcId, "id");
  ...
  NATGateway(nodes, names, ctx, warnings) {
    ...
    nodes.forEach((n) => {
      ...
      const name = names[n.id];
      const r = createRefResolver(names, warnings, n.data?.label || name);
      const subnetRef = r("aws_subnet", cfg.subnetId, "id");
  ...
  RouteTable(nodes, names, ctx, warnings) {
    ...
    nodes.forEach((n) => {
      ...
      const routeBlocks = routes.map((route) => {
        ...
        let gwAttr = null;
        if (targetType === "IGW") {
          const igwRef = r("aws_internet_gateway", route.target, "id");
          gwAttr = igwRef ? rawAttr("gateway_id", igwRef, 2) : null;
        } else if (targetType === "NATGateway") {
          const natRef = r("aws_nat_gateway", route.target, "id");
  ...
  EC2(nodes, names, ctx, warnings) {
    return nodes.map((n) => {
      ...
      const profileRef = cfg.iam_role_id ? r("aws_iam_instance_profile", cfg.iam_role_id, "name") : null;
  ...
  LoadBalancer(nodes, names, ctx, warnings) {
    ...
    nodes.forEach((n) => {
      ...
      const vpcRef = vpcId ? r("aws_vpc", vpcId, "id") : null;
  ...
  ECS(nodes, names, ctx, warnings) {
    ...
    nodes.forEach((n) => {
      ...
      const subnetRefs = r.list("aws_subnet", subnetIds, "id");
      const sgRefs = r.list("aws_security_group", cfg.sg_ids, "id");
      const roleRef = cfg.iam_role_id ? r("aws_iam_role", cfg.iam_role_id, "arn") : null;
  ...
  Route53(nodes, names, ctx, warnings) {
    ...
    nodes.forEach((n) => {
      ...
      if (cfg.zone_type === "private") {
        const vpcs = ctx.vpcs || [];
        if (vpcs.length > 0) {
          const vpcRef = r("aws_vpc", vpcs[0].id, "id");
          if (vpcRef) {
...
function generateIAMRoles(ctx, names) {
  ...
  roles.forEach((role) => {
    ...
    assignedNodes.forEach((n) => {
      ...
      if (rt === "EC2") services.add("ec2.amazonaws.com");
